...
Maintaining two user-facing flavors was the real cost, and the MADR `decide` lifecycle was the sole source of the field-reported bugs; collapsing to lean removes both. The shared parsing core is retained under the surviving rule that the shared parsing primitives are never re-forked — the split that mattered was at the user-facing layer, and now there is only one side of it.
