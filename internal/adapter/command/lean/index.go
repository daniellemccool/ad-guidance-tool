...
category-grouped README index. With --root <tree> it also runs scope lint: stale
applies_to globs (matching no file under the tree) and default-vs-default scope
overlap. Warnings are advisory; a hard validation failure exits non-zero.`,
	...
	cmd.Flags().StringVar(&root, "root", "", "Source tree root for scope lint (stale applies_to + overlap); skipped if empty")
